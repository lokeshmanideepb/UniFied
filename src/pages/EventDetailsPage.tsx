...
const EventDetailsPage: React.FC = () =>
{
  ...
  useEffect( () =>
  {
    const getEvent = async () =>
    {
      if ( !event && eventId )
      {
        let data = await fetchEvent( eventId );
        if ( data )
          setEvent( data );
        else
          navigate( '/events' );
  ...
  return (
    <div className="max-w-4xl p-6 bg-white text-left">
      ...
      <div className="text-lg text-gray-700 mt-6">
        ...
        <p>{event.from_time} {event.to_time ? ` - ${ event.to_time }` : ""}</p>
      </div>
      <button
      ...
      {event.location ?
        <div className="text-lg text-gray-700 mt-6">
          ...
          <p><a href="#" className="text-blue-600 underline">{event.location}</a></p>
        </div>
        : ""}
      ...
      <div className="text-lg text-gray-700 mt-6">
        ...
        <div className="flex space-x-2">
          {event.tags.map( tag => <span className=" text-blue-600">{tag}</span> )}
        </div>
      </div>
